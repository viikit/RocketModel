...
# Equations of motion
# F = m*a = m * zddot
# z iz the altitude from the center of the planet along the north pole
...
# Checking the masses after stage separation and SECO
...
def plotmap():
    ...
    distance1 = electronlpadlon + (down1*np.sin(langle) / Rplanet) * (180 / np.pi) / np.cos(lpadlat * np.pi/180)
    distance1nowind = electronlpadlon + (down1nowind / Rplanet) * (180 / np.pi) / np.cos(lpadlat * np.pi/180)
    mapbox_access_token = open("mapbox_publictoken.txt").read()

    lat1 = electronlpadlat + (down1 * np.cos(langle))/ Rplanet * (180 / np.pi)# / np.cos(27.57 * np.pi/180)# / Rplanet * (180 / np.pi) / np.cos(42.085804 * np.pi/180
    lat3 = electronlpadlat + (down1nowind * np.cos(langle))/ Rplanet * (180 / np.pi)
    lon1 = electronlpadlon + (down1*np.sin(langle) / Rplanet) * (180 / np.pi) #/ (np.cos(lat1) * np.pi/180)
    lon3 = electronlpadlon + (down1nowind * np.sin(langle) / Rplanet) * (180 / np.pi)
...
def velocityplot():
    ...
    plt.plot(tout1+153, velout1, label="Първа степен")
    plt.xlabel('Време [s]')
...
def heightplot():
    ...
    plt.plot(tout1+153, altitude1, label="Първа степен")
    plt.xlabel('Време [s]')
...
def orbitplot():
    ...
    plt.plot(xout[1:F_stage_counter],zout[1:F_stage_counter], 'g-', label="Първа степен")
    plt.plot(xout[F_stage_counter:F_sep_stage_counter],zout[F_stage_counter:F_sep_stage_counter], 'k-', label='Отделяне на степента')
    plt.plot(xout[F_sep_stage_counter:S_stage_counter],zout[F_sep_stage_counter:S_stage_counter], 'r-', label="Втора степен")
